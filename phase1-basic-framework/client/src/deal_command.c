...
static void init_prompt(int sockfd, char *prompt)
{
    static int initialized = 0;
    if(initialized) return;

    snprintf(prompt, PATH_MAX, "%s",  CLOUD_DRIVE_DIR);
    packet_t request;
    memset(&request, 0, sizeof(request));
    request.type = PROMPT;
    request.status = 0;

    if(send_packet(sockfd, &request) == -1) return;
    initialized = 1;
}

static void print_prompt(int sockfd, char *prompt)
{
    init_prompt(sockfd, prompt);
    printf("%s@%s:%s$ ", USER_NAME, SERVICE_NAME, prompt);
    fflush(stdout);
}
