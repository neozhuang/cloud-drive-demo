                                         ...
                                         uint32_t payload_len)
{
    ...
    if (packet_init(&response, type, status, session_id,
                    payload, payload_len) != 0) {
        return -1;
    }

    return protocol_send_packet(task->client_fd, &response);
}

static int send_response(packet_task_t *task,
                         cmd_type_t type,
                         status_code_t status,
                         const void *payload,
                         uint32_t payload_len)
{
    return send_response_with_session_id(task, type, status,
                                         &task->packet.header.session_id,
                                         payload, payload_len);
}

static int copy_payload(packet_task_t *task, char *destination,
                        size_t capacity, int require_nonempty)
{
    uint32_t payload_len = task->packet.header.payload_len;

    if (payload_len >= capacity ||
        (payload_len > 0 && task->packet.payload == NULL) ||
        (require_nonempty && (payload_len == 0 || task->packet.payload == NULL))) {
        return -1;
    }

    if (payload_len > 0) {
        memcpy(destination, task->packet.payload, payload_len);
    }
    destination[payload_len] = '\0';
    return 0;
}
...
void handle_basic_task(void *arg)
{
    ...
    if (task->packet.header.type == CMD_REGISTER_REQ ||
        task->packet.header.type == CMD_LOGIN_REQ) {
        username = "anonymous";
    } else {
    ...
    switch (task->packet.header.type) {
        ...
        default:
            send_response(task, CMD_ERROR, STATUS_PROTOCOL_ERROR, NULL, 0);
            break;
    }
...
static void handle_register(packet_task_t *task)
{
    ...
    // 1. get the username and password from packet.payload 
    if (copy_payload(task, payload, sizeof(payload), 1) != 0) {
        send_response(task, CMD_ERROR, STATUS_BAD_REQUEST, NULL, 0);
        return;
    }
    ...
    if (generate_salt(salt, sizeof(salt)) != 0) {
        send_response(task, CMD_ERROR, STATUS_OTHER_ERROR, NULL, 0);
        return;
    }
    ...
    if (dao_auth_register(task->db_pool, username, password_hash) != 0) {
        // TODO: user has already existed or other error
        send_response(task, CMD_ERROR, STATUS_DB_ERROR, NULL, 0);
        return;
    }

    // 5. return CMD_ACK / CMD_ERROR
    send_response(task, CMD_ACK, STATUS_OK, NULL, 0);
...
static void handle_login(packet_task_t *task)
{
    ...
    // 5. if user login success, insert new session.
    ...
    if (ret == 1) {
        // username not exist
        // 3. if not exist, return CMD_ERROR, STATUS_USER_NOTEXIST
        send_response(task, CMD_ERROR, STATUS_USER_NOTEXIST, NULL, 0);
        return;
    }
    if (ret == -1) {
        // other error 
    ...
    if (encrypted_password == NULL || strcmp(login_info.password_hash, encrypted_password) != 0) {
        send_response(task, CMD_ERROR, STATUS_PASSWD_ERROR, NULL, 0);
        return;
    }
    memset(&data, 0, sizeof(data));

    // 5. Create an authenticated session and return its ID to the client.
    if (session_create(task->session_table, task->client_fd,
                       login_info.user_id, username,
                       login_info.root_path_id, &new_id) != 0) {
        send_response(task, CMD_ERROR, STATUS_OTHER_ERROR, NULL, 0);
        LOG_ERROR("Failed to create login session");
        return;
    }

    // 6. return CMD_ACK / CMD_ERROR - STATUS_PASSWD_ERROR
    send_response_with_session_id(task, CMD_ACK, STATUS_OK, &new_id, NULL, 0);
    LOG_INFO("User %s login success", username);
}

static void handle_pwd(packet_task_t* task)
{
    send_response(task, CMD_ACK, STATUS_OK,
                  task->session.cwd, strlen(task->session.cwd));
}
...
static void handle_cd(packet_task_t* task)
{
    ...
    // 1. get the path from task->payload
    if (copy_payload(task, payload, sizeof(payload), 0) != 0) {
        send_response(task, CMD_ERROR, STATUS_BAD_REQUEST, NULL, 0);
        return;
    }
    ...
    if (payload[0] == '\0') {
        strcpy(virtual_path, "/");
    } else if (normalize_cd_path(task->session.cwd, payload,
                                 virtual_path, sizeof(virtual_path)) != 0) {
        send_response(task, CMD_ERROR, STATUS_OTHER_ERROR, NULL, 0);
        return;
    }
    ...
    int ret = dao_path_get_node_id(task->db_pool, task->session.user_id,
    ...
    switch (ret) {
        ...
        case DAO_PATH_NOT_FOUND:
            send_response(task, CMD_ERROR, STATUS_DIR_NOTEXIST, virtual_path, strlen(virtual_path));
            return;

        case DAO_PATH_IS_FILE:
            send_response(task, CMD_ERROR, STATUS_NOT_DIR, virtual_path, strlen(virtual_path));
            return;
    ...
    // 5. update the cwd and cwd_id at this session
    if (session_update_cwd(task->session_table, task->client_fd,
                           &task->packet.header.session_id,
                           path_id, virtual_path) != 0) {
        send_response(task, CMD_ERROR, STATUS_OTHER_ERROR, NULL, 0);
        return;
    }
    // 6. return ok/error, path not exist
    send_response(task, CMD_ACK, STATUS_OK, virtual_path, strlen(virtual_path));
}
...
static void handle_ls(packet_task_t* task)
{
    ...
    // 3. Normalize ls target path.
    if (payload[0] == '\0') {
        strcpy(target_path, task->session.cwd);
    } else if (normalize_cd_path(task->session.cwd, payload,
    ...
    switch (ret) {
        ...
        case DAO_PATH_NOT_FOUND:
            send_response(task, CMD_ERROR, STATUS_DIR_NOTEXIST, target_path, strlen(target_path));
            return;

        default:
            send_response(task, CMD_ERROR, STATUS_OTHER_ERROR, target_path, strlen(target_path));
            return;
    }
    ...
    if (ret == DAO_PATH_IS_FILE) {
        ...
        send_response(task, CMD_ACK, STATUS_OK, file_name, strlen(file_name));
    }

    // 6. If target path exists and is dir, checkout all child files/dirs.
    if (ret == DAO_PATH_IS_DIR) {
        ls_resp[0] = '\0';
        if (dao_path_get_ls_dir(task->db_pool, task->session.user_id,
                                target_id, ls_resp, sizeof(ls_resp)) < 0) {
            send_response(task, CMD_ERROR, STATUS_OTHER_ERROR, target_path, strlen(target_path));
            return;
        }
        send_response(task, CMD_ACK, STATUS_OK, ls_resp, strlen(ls_resp));
    }
}
...
static void handle_mkdir(packet_task_t* task)
{
    ...
    // 3. Normalize mkdir target path.
    if (normalize_cd_path(task->session.cwd, payload,
                          target_path, sizeof(target_path)) != 0) {
        send_response(task, CMD_ERROR, STATUS_BAD_REQUEST, NULL, 0);
        return;
    }
    ...
    switch (ret) {
        ...
        case DAO_PATH_NOT_FOUND:
            send_response(task, CMD_ERROR, STATUS_DIR_NOTEXIST, parent_path, strlen(parent_path));
            return;

        case DAO_PATH_IS_FILE:
            send_response(task, CMD_ERROR, STATUS_NOT_DIR, parent_path, strlen(parent_path));
            return;

        default:
            send_response(task, CMD_ERROR, STATUS_OTHER_ERROR, parent_path, strlen(parent_path));
            return;
    }

    // 6. Insert new directory.
    ret = dao_path_insert_dir(task->db_pool, task->session.user_id,
                              target_path, parent_id, file_name);
    if (ret == 1) {
        // Directory already exists
        send_response(task, CMD_ERROR, STATUS_DIR_ALREADY_EXISTS, file_name, strlen(file_name));
        return; 
    } else if (ret < 0) {
        send_response(task, CMD_ERROR, STATUS_OTHER_ERROR, file_name, strlen(file_name));
        return; 
    }
...
static void handle_rmdir(packet_task_t* task)
{
    ...
    // 4. Check the target path exists and is directory.
    int ret = dao_path_get_node_id(task->db_pool, task->session.user_id,
                                   target_path, &target_dir_id);
    switch (ret) {
        case DAO_PATH_IS_DIR:
            break;

        case DAO_PATH_NOT_FOUND:
            send_response(task, CMD_ERROR, STATUS_DIR_NOTEXIST, file_name, strlen(file_name));
            return;

        case DAO_PATH_IS_FILE:
            send_response(task, CMD_ERROR, STATUS_NOT_DIR, file_name, strlen(file_name));
            return;
    ...
    ret = dao_path_dir_has_child(task->db_pool, task->session.user_id,
                                 target_dir_id);
    if (ret > 0) {
        // dir is not empty
        send_response(task, CMD_ERROR, STATUS_DIR_NOTEMPTY, file_name, strlen(file_name));
        return;
    } else if (ret < 0) {
        // db error
        send_response(task, CMD_ERROR, STATUS_OTHER_ERROR, file_name, strlen(file_name));
    ...
    // 7. Return success or error
    send_response(task, CMD_ACK, STATUS_OK, target_path, strlen(target_path));
}
...
static void handle_rm(packet_task_t* task)
{
    ...
    ret = dao_path_get_node_id(task->db_pool, task->session.user_id,
                               target_path, &target_file_id);
    switch (ret) {
        case DAO_PATH_IS_FILE:
            break;

        case DAO_PATH_IS_DIR:
            send_response(task, CMD_ERROR, STATUS_IS_DIR, base_file_name, strlen(base_file_name));
            return;

        case DAO_PATH_NOT_FOUND:
            send_response(task, CMD_ERROR, STATUS_FILE_NOTEXIST, base_file_name, strlen(base_file_name));
            return;

        default:
            send_response(task, CMD_ERROR, STATUS_OTHER_ERROR, base_file_name, strlen(base_file_name));
            return;
    }

    // 5. Remove the path record and decrement the physical file reference.
    ret = dao_rm_file(task->db_pool, task->session.user_id, target_file_id,
                      hashed_filename, sizeof(hashed_filename));
    switch (ret) {
        case DAO_OK:
            break;
        ...
        case DAO_SHOULD_DELETE_PHYSICAL:
            ...
            if (unlink(physical_file_path) != 0) {
                ...
                send_response(task, CMD_ERROR, STATUS_IO_ERROR, base_file_name, strlen(base_file_name));
            }
            break;
    ...
    send_response(task, CMD_ACK, STATUS_OK, base_file_name, strlen(base_file_name));
}
