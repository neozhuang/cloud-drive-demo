...
void handle_accept_event(int epoll_fd, int listen_fd)
{
    ...
    if (client_fd == -1) {
        LOG_WARN("Failed to accept client connection, errno = %d", errno);
        return;
    }
...
void handle_client_event(int epoll_fd, int client_fd, uint32_t events, thread_pool_t *thread_pool)
{
